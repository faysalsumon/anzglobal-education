...
-   **AI Chat Agent with RAG Knowledge Base**: Floating chat widget available on all public and authenticated pages featuring custom **animated 3-books icon** representing knowledge and learning. Icon uses smooth CSS transform-based animations where top and bottom bars rhythmically open and close (2-second loop) similar to Replit's animated branding. Features RAG-powered AI assistant using Pinecone vector database for platform-specific knowledge (courses, institutions, campuses, platform guides). Backend uses OpenAI GPT-4o-mini with strict system prompts preventing hallucinations and mandatory accuracy disclaimers. Knowledge base extracts approved courses/institutions and creates embeddings using text-embedding-3-small (1536 dimensions). **Event-Driven Auto-Updates**: Knowledge base automatically rebuilds when courses/institutions are approved, rejected, edited, deleted, or status-toggled via 11 admin endpoints using fire-and-forget async pattern with success/error logging. Auto-initialization feature populates knowledge base on first query with approved content and platform guides. Chat widget includes minimizable interface, unread indicators, typing animations, markdown rendering for rich responses, and source citations. Custom SVG favicon with matching 3-books design. Supports both authenticated (session-based) and anonymous users. Database schema tracks conversations and message history. Admin endpoints for knowledge base management (/api/chat/admin/build-knowledge-base, /api/chat/admin/test-query).
