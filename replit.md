...
- **Accounting / Finance Module**: Full accounting module modelled after Zoho Books/QuickBooks. Includes Chart of Accounts (pre-seeded with 20 accounts across 5 types), Customer directory, Items catalog, Invoice management (auto-numbered per region: ANZ-AU-0001, ANZ-BD-0001), Payment recording, Credit notes, and transactional emails (invoice sent, payment receipt, overdue reminder). Admin sidebar has a "Finance" group with Dashboard, Invoices, Customers, Items, and Chart of Accounts sub-items. DB tables: acc_chart_of_accounts, acc_customers, acc_items, acc_invoices, acc_invoice_line_items, acc_payments_received, acc_credit_notes, acc_credit_note_items. Routes in server/accounting-routes.ts. Email functions in server/email-service.ts.
