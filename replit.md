...
- **People / HR Module (Attendance)**: New "People" domain in admin sidebar (teal, `UsersRound` icon) serving as the HR/team management hub. First feature: Attendance — clock-in/out with webcam photo capture, time tracking, RBAC-scoped reports. DB table: `attendance_records` (id, userId, branchId, clockInAt, clockInPhotoPath, clockOutAt, clockOutPhotoPath, totalMinutes, workDate, notes, createdAt). Photos stored in object storage at `attendance-photos/{userId}/{timestamp}-{in|out}.jpg`, proxied at `/api/public-storage/attendance-photos/:userId/:filename`. Backend: `server/attendance-routes.ts` — clock-in, clock-out, status, admin list+stats, cleanup. Frontend: `ClockInButton` in header (status dot, elapsed timer, opens `WebcamCaptureModal`), `AttendancePanel` (stats cards, date/user/branch filters, records table with photo thumbnails). RBAC: CTO/platform_admin → all branches; branch managers → own branch; staff → own records. Designed to expand into Leave Management, KPI Tracking, and Performance Reviews.
...
- **Course-Level Visibility**: Courses have a `visibility` field (`'public'` | `'private'`, default `'public'`) on the `courses` table. Private courses are only visible to logged-in students (student portal sends `includePrivate=true`). Course editor has a "Publish" dropdown with "Publish Publicly" and "Publish Privately" options (mirrors institution editor). Admin dashboard course list shows an amber "Private" badge with lock icon for private courses and has "Publish Publicly" / "Publish Privately" dropdown options. Filtering applied to `/api/courses`, `/api/courses/:id`, `/api/disciplines`, and `/api/course-levels` endpoints. Institution-level privacy still applies independently (private institution hides all its courses regardless of course visibility).
- **Team Channels (Cliq-style Messaging)**: Full Slack/Cliq-style messaging in admin `Messages` tab. DB tables: `channels`, `channel_members`, `channel_messages`. API: `GET/POST /api/channels`, `POST /api/channels/:id/join`, `GET/POST /api/channels/:id/messages`, `GET /api/channels/public`. UI redesign of `admin-messages-tab.tsx`: sidebar with Channels + DMs sections; Zan AI bot pinned at top of DMs; presence dots using existing `availabilityStatus`; date separators; consecutive message grouping; read receipt `✓✓`; attachment panel (+→Photos/File/DocumentScanner/Location/Contact); emoji picker; mic button (UI only); real-time via WebSocket `channel_message` events. `#general` channel seeded with all admin users.
- **Task System (Enhanced)**: `task_notes` table stores per-task comment threads with `taskId`, `authorId`, `content`, `mentionedUserIds` (array), `createdAt`. `/api/tasks/my-tasks` now returns tasks where user is **assignee OR creator** (not just assignee). `GET /api/tasks/:id/notes` and `POST /api/tasks/:id/notes` endpoints serve the comment thread. Task dialog shows a "Notes & Updates" thread when editing with @mention support — typing `@` opens a filtered dropdown of team members, selecting inserts `@Name` and stores their ID. `my-tasks-panel.tsx` shows a `→ Assignee` chip when you delegated the task, and `from Creator` when someone assigned it to you.
...
- **Google Tag Manager**: GTM container `GTM-NTPF5HRG` installed in `client/index.html`. Head script placed as the first element in `<head>`; noscript iframe fallback placed as the first element in `<body>`. GTM manages GA4 entirely — the duplicate direct `gtag/js` script was removed to eliminate 138KB of redundant JS. Only the inline `window.dataLayer` and `function gtag(){}` setup remain for SPA tracking compatibility.
- **PageSpeed Optimisations**: Multi-sprint effort targeting PageSpeed 95+. Key changes: (1) Public routes in App.tsx bypass the auth-loading spinner — footer renders on first paint eliminating 0.42 CLS; (2) ANZ logo converted from 47.8KB PNG to 7.4KB WebP with explicit CSS dimensions (no `w-auto`); (3) Google Fonts switched from `display=swap` to `display=optional` to eliminate font-swap CLS; (4) hreflang tags added to index.html for BD/AU regional SEO; (5) Skeleton card heights matched to real card heights to prevent replacement CLS; (6) All `bg-accent text-white` changed to `bg-accent text-accent-foreground` for WCAG AA contrast compliance (orange + dark text = ~4.9:1 ratio); (7) Cloudflare actions needed for bfcache — HTML page returning `no-store` is a Cloudflare-side setting.
- **WCAG AA Colour Contrast**: All orange-accent (`bg-accent`) buttons and badges now use `text-accent-foreground` (hsl(20,60%,8%) = dark brown-black) instead of `text-white`. Contrast ~4.9:1 vs required 4.5:1. Applies to landing.tsx, public-header.tsx, public-mobile-nav.tsx.
