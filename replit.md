...
- **CRM System (Phase 1)**: Task assignment and workload management for consultants and admins. Features include:
  ...
  - **Internal Notes**: Application-level threaded discussions for team communication (applicationInternalNotes table), displayed in ApplicationInternalNotes component within application details dialog. Notes support pinning and author-only deletion.
  ...
  - **Admin Dashboard Integration**: My Tasks tab displays MyTasksPanel (2/3 width) + UpcomingRemindersPanel (1/3 width) side by side. Team Workload tab (admin-only) shows TeamWorkloadPanel for workload distribution visibility.
