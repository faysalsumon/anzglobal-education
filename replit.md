...
The backend is a Node.js Express.js server written in TypeScript. Authentication uses OpenID Connect (OIDC) via Replit Auth and Passport.js, with sessions stored in PostgreSQL. The API is RESTful, organized by user type, including endpoints for authentication, university management (profile, courses, applications, team), student operations (profile, applications, AI-powered course recommendations), public course listings, and super admin functionalities for comprehensive CRUD operations on users, institutions, and courses. A dedicated `RecommendationService` module (`server/recommendations.ts`) implements the course matching algorithm with eligibility filtering, weighted scoring, and AI enhancement. A data access layer (`IStorage` interface) abstracts database operations. AI integration uses the OpenAI API via Replit's AI Integrations service (GPT-4o-mini for recommendations, GPT-5 for content generation) for generating university and course descriptions, student bios, and personalized course recommendation insights. Shared Zod schemas (`shared/schema.ts`) ensure type-safe validation across client and server. Image processing for uploads and AI-generated content utilizes Multer and Sharp.

### Data Storage

PostgreSQL, accessed via Neon's serverless driver, is the primary database. Drizzle ORM is used for schema-first design and interactions, with Drizzle Kit managing migrations. The schema includes tables for sessions, users (distinguishing universities from students and admins), university profiles (with extended fields like `smallDescription`, `fullDescription`, `institutionGallery`, `topCourses`), team members with role-based permissions, courses (with detailed fields like `courseCode`, `prPathway`, `scholarshipPercentage`, `eligibilityRequirements`, `englishRequirements`, `curriculumUrl`, `costOfLiving`, `applicationFees`, `images`, `intakeMonths`, `reviews`), student profiles (with matching criteria fields: `budgetMin`, `budgetMax`, `preferredSubjects`, `preferredCountries`, `preferredIntakeMonths`, `desiredScholarship`, `englishScore`, `workExperienceYears`, `studyModePreference` for AI-powered course matching), applications, referral tracking, favorites (with `itemType` enum for 'university' or 'course', composite unique constraint on userId/itemType/itemId, and proper foreign key relations), and course comparisons (enabling students to compare up to 4 courses side-by-side with a unique index on studentProfileId/courseId). Replit Object Storage is configured for institution and course images.
