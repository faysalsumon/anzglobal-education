...
## Replit fallback — REMOVED (all 8876 files confirmed in Supabase)
All files are in Supabase (migration confirmed 0 copied · 8876 skipped). `downloadFile()` is Supabase-only. Do not re-add Replit fallback — it conflicts with the Railway migration task requirement to eliminate the `@replit/object-storage` runtime dependency.
